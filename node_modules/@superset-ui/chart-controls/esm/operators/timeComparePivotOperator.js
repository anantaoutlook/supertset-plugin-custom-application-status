...
export const timeComparePivotOperator = (formData, queryObject) => {
  ...
  if (isValidTimeCompare(formData, queryObject)) {
    ...
    const changeAgg = Object.fromEntries([...metricOffsetMap.entries()].map(([offset, metric]) => [comparisonType, metric, offset].join(TIME_COMPARISON_SEPARATOR)) // use the 'mean' aggregates to avoid drop NaN
    .map(metric => [metric, {
      operator: 'mean'
    }]));
    return {
